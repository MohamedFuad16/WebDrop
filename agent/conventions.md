...
**Imports:** relative paths with a cache-busting query: `import { X } from "../utils/foo.js?v=1.0.92";`. Keep the `?v=<APP_VERSION>` suffix consistent with `package.json` version and `service-worker.js` (bump together). No bare/package specifiers in browser code (deps like `jsqr`/`qrcode-generator` are vendored under `js/vendor/`).
