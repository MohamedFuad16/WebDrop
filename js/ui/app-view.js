...
export class AppView extends Emitter {
  ...
  syncThemeColor(theme) {
    const themeColor = theme === "dark" ? "#171818" : "#f3f3f1";
...
function deviceBrand(peer) {
  ...
  if (family === "ipad" || /(ipad|tablet|tab|pad|fold)/.test(label)) return "apple";
  if (family === "ios" || /(iphone|apple)/.test(label)) return "apple";
  if (/(surface|windows)/.test(label)) return "windows";
