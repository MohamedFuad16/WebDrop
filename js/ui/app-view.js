...
import { AVATAR_OPTIONS, animatedFramesForAvatar, normalizeAvatarChoice } from "../config/avatar-options.js?v=1.0.127";
import { translate } from "../config/i18n.js?v=1.0.127";
import { isPreviewableReceivedItem } from "../utils/received-files.js?v=1.0.127";
import { DynamicIsland } from "./dynamic-island.js?v=1.0.127";
import { OnboardingTour } from "./onboarding.js?v=1.0.127";
...
export class AppView extends Emitter {
  constructor(document, store) {
    ...
    // First launch only: open the guide unprompted, then remember it was shown so it
    // never reappears (the how-to FAB stays the way back in). Deferred a frame because
    // open() measures slide geometry, which is only correct after the first render.
    if (!localStorage.getItem("webdrop.onboardingSeen")) {
      localStorage.setItem("webdrop.onboardingSeen", "true");
      globalThis.requestAnimationFrame(() => this.onboarding.open());
    }
  ...
  showTabReplaced() {
    const overlay = this.document.querySelector("[data-tab-replaced]");
  ...
  updateIslandPeerProfile(peer) {
    this.dynamicIsland.updatePeerProfile?.(peer);
  }
