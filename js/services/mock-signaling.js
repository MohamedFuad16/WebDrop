...
import { AVATAR_OPTIONS } from "../config/avatar-options.js?v=1.0.109";
...
export class MockSignalingAdapter extends Emitter {
  ...
  async joinProximitySession({ clientNonce } = {}) {
    ...
    queueMicrotask(() => {
      ...
      setTimeout(() => {
        const acousticPlan = [
          { id: "mock-self", startFrequencyHz: 18600, endFrequencyHz: 19400, code: 0 },
          { id: "mock-peer", startFrequencyHz: 18600, endFrequencyHz: 19400, code: 1 }
        ];
        this.emit("proximity:session:start", {
          ...
          durationMs: 2600,
          acousticSlot: 0,
          acousticSignatureId: acousticPlan[0].id,
          acousticPlan,
          participantCount: 2
        });
      }, 0);
