...
COPY = {
    "en": {
        ...
        "cover_summary": (
            "A visual and technical guide to the WebDrop 1.0.34 prototype. "
        ...
        "overview_cards": [
            ...
            (
                ...
                "Version 1.0.34 proves the mobile-first interaction, responsive orbit "
        ...
        "limits_intro": (
            "The interface and networking stack are locally verified, but the public Azure "
            "endpoint and physical-device proximity still determine launch readiness."
        ),
        "limits_cards": [
            (
                "Public signaling",
                "Production WebSocket signaling works locally, while the configured Japan East "
                "VM must be restored before public multi-device discovery is available.",
            ),
            (
                "TURN is locally verified",
                "Managed Cloudflare TURN credentials and forced relay pass locally. The same "
                "proof must be repeated through the restored public endpoint.",
            ),
            (
                "Permissions vary",
        ...
        "catalog_subtitle": "Approved WebDrop 1.0.34 UI inventory",
    ...
    "ja": {
        ...
        "cover_summary": (
            "WebDrop 1.0.34の画面と技術をまとめたデモガイドです。"
        ...
        "overview_cards": [
            ...
            (
                "このビルドで確認できること",
                "バージョン1.0.34では、モバイル中心の操作、レスポンシブな軌道UI、日英切替、"
        ...
        "limits_intro": (
            "画面と通信機能はローカルで検証済みですが、公開Azureエンドポイントと実機近接検証が本番公開の条件です。"
        ),
        "limits_cards": [
            (
                "公開シグナリング",
                "本番WebSocket通信はローカルで動作確認済みです。公開端末検索にはJapan East VMの復旧が必要です。",
            ),
            (
                "TURNはローカル検証済み",
                "Cloudflare TURN認証と強制中継はローカルで成功しています。"
                "復旧後の公開エンドポイントでも同じ検証を行います。",
            ),
            (
                "権限は端末ごとに異なる",
        ...
        "catalog_title": "UIガイド",
        "catalog_subtitle": "承認済みWebDrop 1.0.34画面一覧",
        "roadmap_title": "今後の予定",
        "roadmap_statement": (
...
JP_ITEMS = {
    ...
    "settings-app-version": ("アプリバージョン", "設定画面の下部に現在のプロトタイプバージョン1.0.34を表示します。"),
...
def draw_cover(
    ...
    c.setFillColor(WHITE)
    c.setFont(bold, 9)
    c.drawCentredString(MARGIN + 45.5, PAGE_H - 350, "VERSION 1.0.34")
...
def build_pdf(locale: str, items: Sequence[dict[str, str]], fonts: dict[str, str]) -> Path:
    ...
    c.setAuthor("WebDrop")
    c.setSubject("WebDrop 1.0.34 interface and architecture guide")
